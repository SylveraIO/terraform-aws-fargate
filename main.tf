...
module "vpc" {
  ...
  name = "${var.name}-${var.env_name}-vpc"
  cidr = var.vpc_cidr
...
resource "aws_ecr_repository" "this" {
  ...
  name = "${local.services[count.index].name}-${var.env_name}"
}
...
resource "aws_ecs_cluster" "this" {
  name = "${var.name}-${var.env_name}-cluster"
}

# ECS TASKS DEFINITIONS

resource "aws_iam_role" "tasks_execution" {
  name               = "${var.name}-${var.env_name}-task-execution-role"
  assume_role_policy = file("${path.module}/policies/ecs-task-execution-role.json")
}

resource "aws_iam_policy" "tasks_execution" {
  name = "${var.name}-${var.env_name}-task-execution-policy"
...
resource "aws_iam_policy" "tasks_execution_ssm" {
  ...
  name = "${var.name}-${var.env_name}-task-execution-ssm-policy"
...
resource "aws_ecs_task_definition" "this" {
  ...
  family                   = "${var.name}-${var.env_name}-${local.services[count.index].name}"
...
resource "aws_security_group" "web" {
  vpc_id = local.vpc_id
  name   = "${var.name}-${var.env_name}-web-sg"
...
resource "aws_security_group" "services" {
  ...
  vpc_id = local.vpc_id
  name   = "${var.name}-${local.services[count.index].name}-${var.env_name}-services-sg"
...
resource "aws_security_group" "services_dynamic" {
  ...
  vpc_id = local.vpc_id
  name   = "${var.name}-${local.services[count.index].name}-${var.env_name}-services-sg-dynamic"
  ...
  dynamic "ingress" {
    ...
    content {
      ...
      if lookup(s, "name", "") == "${var.name}-${ingress.value}-${var.env_name}-services-sg"]
    }
  }
}
...
resource "aws_lb" "this" {
  ...
  name            = "${var.name}-${var.env_name}-${local.services[count.index].name}-alb"
...
resource "aws_service_discovery_private_dns_namespace" "this" {
  ...
  name        = "${var.name}.${var.env_name}.local"
...
resource "aws_iam_role" "autoscaling" {
  name               = "${var.name}-${var.env_name}-appautoscaling-role"
  assume_role_policy = file("${path.module}/policies/appautoscaling-role.json")
}

resource "aws_iam_role_policy" "autoscaling" {
  name   = "${var.name}-${var.env_name}-appautoscaling-policy"
...
resource "aws_s3_bucket" "this" {
  bucket        = "${var.name}-${var.env_name}-builds"
  acl           = "private"
  force_destroy = true
}

resource "aws_iam_role" "codebuild" {
  name               = "${var.name}-${var.env_name}-codebuild-role"
...
resource "aws_iam_role_policy" "codebuild" {
  name   = "${var.name}-${var.env_name}-codebuild-role-policy"
...
resource "aws_codebuild_project" "this" {
  ...
  name          = "${var.name}-${var.env_name}-${local.services[count.index].name}-builds"
  build_timeout = "10"
...
resource "aws_iam_role" "codepipeline" {
  ...
  name = "${var.name}-${var.env_name}-${local.services[count.index].name}-codepipeline-role"
...
resource "aws_iam_role_policy" "codepipeline" {
  ...
  name   = "${var.name}-${var.env_name}-${local.services[count.index].name}-codepipeline-role-policy"
...
resource "aws_codepipeline" "this" {
  ...
  name     = "${var.name}-${var.env_name}-${local.services[count.index].name}-pipeline"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ProjectName = "${var.name}-${var.env_name}-${local.services[count.index].name}-builds"
      }
    }
  }
...
resource "aws_cloudwatch_event_rule" "codepipeline_events" {
  ...
  name        = "${var.name}-${var.env_name}-pipeline-events"
...
resource "aws_sns_topic" "codepipeline_events" {
  ...
  name         = "${var.name}-${var.env_name}-codepipeline-events"
  display_name = "${var.name}-${var.env_name}-codepipeline-events"
}
...
resource "aws_cloudwatch_event_target" "codepipeline_events" {
  ...
  target_id = "${var.name}-${var.env_name}-codepipeline"
  arn       = aws_sns_topic.codepipeline_events[count.index].arn
}
...
resource "aws_cloudwatch_dashboard" "this" {
  ...
  dashboard_name = "${var.name}-${var.env_name}-${local.services[count.index].name}-metrics-dashboard"
...
resource "aws_iam_role" "events" {
  ...
  name = "${var.name}-${var.env_name}-${local.services[count.index].name}-events-role"
...
resource "aws_iam_role_policy" "events" {
  ...
  name   = "${var.name}-${var.env_name}-${local.services[count.index].name}-events-role-policy"
...
resource "aws_cloudwatch_event_rule" "events" {
  ...
  name        = "${var.name}-${var.env_name}-${local.services[count.index].name}-ecr-event"
...
resource "aws_cloudwatch_event_target" "events" {
  ...
  target_id = "${var.name}-${var.env_name}-${local.services[count.index].name}-codepipeline"
